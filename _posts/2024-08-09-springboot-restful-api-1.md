...
> **CRUD**는 다음과 같은 네 가지 기본적인 데이터 조작 작업을 의미한다.
> - **C**reate (생성): 새로운 데이터를 데이터베이스에 추가
> - **R**ead (읽기): 데이터베이스에 저장된 데이터를 읽어오는 작업
> - **U**pdate (수정): 기존 데이터를 수정하는 작업
> - **D**elete (삭제): 데이터를 데이터베이스에서 삭제하는 작업
{: .prompt-tip }
...
> 전체 코드는 [여기에서](https://github.com/suhyeoonn/book-rating-backend/tree/chapter1-end) 확인 가능합니다. (chapter1-end 브랜치)
{: .prompt-info }